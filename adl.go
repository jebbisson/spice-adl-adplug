...
// Original code is LGPL-2.1. See LICENSE and THIRD_PARTY_LICENSES.md.

// Package adplugadl implements a parser and player for the Westwood Studios ADL
...
func (ri RawInstrument) ToInstrument(name string) *Instrument {
	return &Instrument{
		Name: name,
		Op1: Operator{
...
// them to Instrument values. Each instrument is named with the given
// prefix and its index (e.g., "dune2_000").
func (f *File) ExtractInstruments(prefix string) []*Instrument {
	...
	for i := 0; i < f.NumPrograms; i++ {
		...
		instruments = append(instruments, ri.ToInstrument(name))
	}
	return instruments
}
