...
func NewPlayer(sampleRate int, file *File) *Player {
	opl := NewBackend(sampleRate)
	runtime := NewRuntime(opl)
	runtime.SetVersion(file.Version)
	runtime.SetSoundData(file.SoundData)
	runtime.InitDriver()
...
func (p *Player) Stop() {
	...
	p.runtime.StopAllChannels()
	p.opl.Reset()
	p.runtime.InitDriver()
	p.tickAccumulator = 0
}
...
func (p *Player) ChannelStates() []ChannelState {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.runtime.SnapshotChannels()
}
...
func (p *Player) rewindLocked(subsong int) {
	...
	if trackID >= 0 {
		p.runtime.StartSound(trackID, 0xFF)
	}
}
...
func (p *Player) isPlaying() bool {
	for i := 0; i < 10; i++ {
		if p.runtime.IsChannelPlaying(i) && !p.runtime.IsChannelRepeating(i) {
			return true
		}
	}
	return false
